<script>
...

                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        if (data.propuestas == 0) {
                            var urle = '<?= site_url() ?>' + 'editar-item';

                            button += '<form  style="display : inline;" action="' + urle + '" method="post" id="formedit' + data.id_item + '">';
                            button += '<input type="hidden" name="item" value="' + data.id_item + '">';
                            button += '<button type="submit" data-tooltip="Editar Item" class="palette-Celeste bg button" form="formedit' + data.id_item + '">';
    ...
    function notificarNovedades(){
        var result = confirm("Quiere Notificar a Todos los Proveedores El Registro de Nuevo Ítems a La Espera De Propuestas?"); 
        if (result == true) { 
